...
export function VerificationBadge({ lastVerified, verifiedBy, compact = false }: VerificationBadgeProps) {
  ...
  const isBroken = verifiedBy === 'auto-check-broken'

  // If link is broken, show broken badge regardless of age
  if (isBroken) {
    if (compact) {
      return (
        <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-earth text-xs font-medium border text-clay bg-clay/10 border-clay/20" title="Link could not be verified">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    return (
      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-earth text-sm font-medium border text-clay bg-clay/10 border-clay/20">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
        </svg>
